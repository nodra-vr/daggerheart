...
export class DiceCustomizationHelper {
	 ...
	 */
	static async applyDiceCustomization(
		roll,
		diceTypes,
		hopeFaces = 'd12',
		fearFaces = 'd12',
		...
		try {
			...
			roll.dice.forEach((die, index) => {
				try {
					...
					if (diceType && this._isValidDiceType(diceType) && presets[diceType]) {
						...
						console.warn(
							`DiceCustomizationHelper: Invalid dice type '${diceType}' or missing preset for die ${index + 1}`
						);
					}
				} catch (dieError) {
			...
			console.debug(
				`DiceCustomizationHelper: Successfully applied customization to ${appliedCount}/${roll.dice.length} dice`
			);
		} catch (error) {
	...
	static _validateDiceTypesArray(diceTypes, expectedLength) {
		...
		if (diceTypes.length !== expectedLength) {
			console.error(
				`DiceCustomizationHelper: diceTypes array length (${diceTypes.length}) must match roll dice length (${expectedLength})`
			);
			return false;
		}
	...
	static _validateAndSanitizeDiceConfig(config, diceType) {
		...
		colorFields.forEach(field => {
			if (this._isValidHexColor(config[field])) {
				...
				console.warn(
					`DiceCustomizationHelper: Invalid ${field} color for ${diceType} dice: ${config[field]}, using default`
				);
	...
	static _validateColorsetConfig(config, diceType = 'unknown') {
		...
		try {
			...
			if (validationErrors > 0) {
				console.warn(
					`DiceCustomizationHelper: Fixed ${validationErrors} validation errors for ${diceType} dice configuration`
				);
			} else {
