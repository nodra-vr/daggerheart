...
export class DaggerheartMeasuredTemplate extends foundry.canvas.placeables.MeasuredTemplate {
	...
	static getDistanceLabel(distance) {
		...
		if (
			canvas.grid.units == 'mi' ||
			canvas.grid.units == 'km' ||
			canvas.grid.units == 'miles' ||
			canvas.grid.units == 'kilometers' ||
			canvas.grid.units == 'miile' ||
			canvas.grid.units == 'kilometer'
		) {
			return '';
		}
		if (distance <= melee) {
...
export class DaggerheartRuler extends foundry.canvas.interaction.Ruler {
	_getWaypointLabelContext(waypoint, state) {
		...
		if (enabled) {
			const distance = DaggerheartMeasuredTemplate.getDistanceLabel(waypoint.measurement.distance.toNearest(0.01));
			if (distance) {
...
export function DaggerheartTemplateEnricher(match, _options) {
	...
	parts.forEach(part => {
		...
		if (split.length === 2) {
			switch (split[0]) {
				case 'type':
					const matchedType = Object.values(TEMPLATE_TYPES).find(x => x.toLowerCase() === split[1]);
					type = matchedType;
					break;
				case 'range':
...
export const renderMeasuredTemplate = async event => {
	...
	const angle =
		type === TEMPLATE_TYPES.CONE
			? CONFIG.MeasuredTemplate.defaults.angle
			: type === TEMPLATE_TYPES.INFRONT
				? 180
				: undefined;

	const baseDistance = game.settings.get(
		'daggerheart-unofficial',
		`rangeMeasurement${range.charAt(0).toUpperCase() + range.slice(1)}`
	);
	...
	static _measureVolumeToVolume(token1, token2, verticalMode) {
		...
		for (const point1 of points1) {
			for (const point2 of points2) {
				...
				const combinedDistance = this._combineDistances(horizontalDistance, point1.z || 0, point2.z || 0, verticalMode);
