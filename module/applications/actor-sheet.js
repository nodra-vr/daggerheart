...
export class SimpleActorSheet extends foundry.appv1.sheets.ActorSheet {
	...
	async getData(options) {
		...
		context.inventoryHTML = await foundry.applications.ux.TextEditor.implementation.enrichHTML(
		...
		context.advancementHTML = await foundry.applications.ux.TextEditor.implementation.enrichHTML(
			context.systemData.advancementnotes,
			{
				secrets: this.document.isOwner,
				async: true,
			}
	...
	async activateListeners(html) {
		...
		html
			.find('.advancement-toggle-row input[type="checkbox"]')
			.on('change', this._onAdvancementCheckboxChange.bind(this));
...
await game.daggerheart.rollHandler.dualityWithDialog({
  title: "Duality Dice Roll"
	...
	async _onAdvancementCheckboxChange(event) {
		...
		try {
			console.log('AdvancementCheckboxChange', { name, checked });
		} catch {}
		...
		if (name.includes('.tier3.proficiency')) {
			...
			if (checked) {
				...
				try {
					console.log('Tier3 proficiency exclusivity', { name, unchecked: other.map((i, e) => e.name).get() });
				} catch {}
			}
		}
		...
		if (name.match(/\.tier[234]\.traits(1|2|3)$/) && checked) {
			...
			try {
				console.log('Trait selection dialog result', { traitKeys });
			} catch {}
			if (Array.isArray(traitKeys) && traitKeys.length > 0) {
				...
				if (match) {
					...
					try {
						console.log('Traits add start', { tierKey, slotName, prevArray });
					} catch {}
					for (const prevTrait of prevArray) {
						const prevId = `adv_trait_${tierKey}_${slotName}_${prevTrait}`;
						try {
							console.log('Removing previous slot modifier', { prevTrait, prevId });
						} catch {}
						...
						if (!prevHasAnyAdvTrait) {
							...
							try {
								console.log('Unset trait levelled after removing last adv', { trait: prevTrait });
							} catch {}
						}
					}
					...
					try {
						console.log('Traits saved to actor', { path: `system.advancements.${tierKey}.${slotKey}`, chosen });
					} catch {}
					for (const t of chosen) {
						const id = `adv_trait_${tierKey}_${slotName}_${t}`;
						try {
							console.log('Adding trait modifier', { trait: t, id });
						} catch {}
						await window.ModifierManager.addModifier(this.actor, `system.${t}.value`, {
							name: `Advancement: ${t}`,
							value: 1,
							permanent: true,
							id,
				...
				input.checked = false;
				try {
					console.warn('Trait selection cancelled or empty; reverting checkbox', { name });
				} catch {}
			}
		}
		...
		if (name.match(/\.tier[234]\.traits(1|2|3)$/) && !checked) {
			...
			if (match) {
				...
				try {
					console.log('Traits uncheck start', { tierKey, slotName, arr });
				} catch {}
				for (const traitKey of arr) {
					const id = `adv_trait_${tierKey}_${slotName}_${traitKey}`;
					try {
						console.log('Removing trait modifier', { traitKey, id });
					} catch {}
					...
					if (!hasAnyAdvTrait) {
						...
						try {
							console.log('Unset trait levelled after full removal', { trait: traitKey });
						} catch {}
					}
				}
				const update = {};
				...
				await this.actor.update(update);
				try {
					console.log('Cleared slot trait array', { path: `system.advancements.${tierKey}.${slotKey}` });
				} catch {}
			}
		}
		...
		if (name.endsWith('.evasion')) {
			...
			if (checked) {
				try {
					console.log('Adding evasion modifier', { slotId });
				} catch {}
				await window.ModifierManager.addModifier(this.actor, 'system.defenses.evasion', {
					name: modifierName,
					value: 1,
					permanent: true,
					id: slotId,
				...
				try {
					console.log('Removing evasion modifier', { slotId });
				} catch {}
		...
		if (name.match(/\.tier[234]\.hp[12]$/)) {
			...
			try {
				console.log('HP max change', { from: currentMax, to: newMax, delta });
			} catch {}
		...
		if (name.match(/\.tier[234]\.stress[12]$/)) {
			...
			const newMax = Math.max(1, currentMax + delta);
			try {
				console.log('Stress max change', { from: currentMax, to: newMax, delta });
			} catch {}
		...
		try {
			console.log('Advancement submit complete');
		} catch {}
	}
	...
	_getTargetingResults(attackTotal) {
		...
		for (let target of game.user.targets) {
			...
			const isCritical = this.getPendingRollType() === 'attack' && (this._lastRollResult?.isCrit || false);
	...
	async _onToggleCategory(event) {
		...
		const categoryList = this.element.find(
			`.item-list[data-location="${dataType}"], .adversaries-grid[data-location="${dataType}"]`
		);
	...
	async _onResourceControl(event) {
		...
		if (updateValue !== undefined) {
			...
			if (
				(resourceName === 'health' || resourceName === 'stress' || resourceName === 'hope') &&
