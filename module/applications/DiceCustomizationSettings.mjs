export class DiceCustomizationSettings extends FormApplication {
			...
			for (const diceType of diceTypes) {
				try {
					const diceData = {
						...
						background:
							this._validateColor(formData[`${diceType}.background`]) || this._getDefaultColor(diceType, 'background'),
		...
		try {
			...
			for (const [key, texture] of Object.entries(textures)) {
				options.push({
					...
					label:
						texture.name ||
						`DAGGERHEART.SETTINGS.DiceCustomization.texture${key.charAt(0).toUpperCase() + key.slice(1)}`,
				});
			}
	...
	_getAvailableSystems() {
		if (!game.dice3d) {
			return [{ value: 'standard', label: 'DAGGERHEART.SETTINGS.DiceCustomization.systemStandard' }];
		}
		...
		try {
			...
			for (const [key, system] of systems.entries()) {
				options.push({
					value: key,
					label:
						system.name || `DAGGERHEART.SETTINGS.DiceCustomization.system${key.charAt(0).toUpperCase() + key.slice(1)}`,
				});
			}
