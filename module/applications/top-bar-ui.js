...
export class TopBarUI {
	...
	async _refreshActiveEnvironment() {
      ...
      `;
			...
			const actionItems = (actor?.items || []).filter(
				i => i.system?.location === 'actions' || (!i.system?.location && i.type === 'item')
			);
			const enriched = await Promise.all(
				actionItems.map(async i => ({
					id: i.id,
					name: i.name,
					img: i.img,
					system: i.system,
					enrichedDescription: await foundry.applications.ux.TextEditor.implementation.enrichHTML(
						...
						{ secrets: actor.isOwner, async: true }
					),
					type: i.type,
				}))
			);
			actionsBody.innerHTML = enriched.length
				? `<ul class="env-actions">${enriched
						.map(
							i => `
			...
			for (const ref of advRefs) {
				try {
					const a = await fromUuid(ref.uuid);
					if (a) advActors.push(a);
				} catch {}
			}
		...
		if (advContainer) {
			advContainer.querySelectorAll('.adv-chip').forEach(chip => {
				...
				chip.addEventListener('click', async () => {
					try {
						const a = await fromUuid(chip.dataset.actorUuid);
						a?.sheet?.render(true);
					} catch {}
				});
			});
